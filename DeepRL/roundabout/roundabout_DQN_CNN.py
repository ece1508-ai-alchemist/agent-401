...
def roundabout_cnn_config_env(env):
    env.configure(
        {
            "observation": {
...
def roundabout_cnn_train_env():
    env = gym.make("roundabout-v0")
    roundabout_cnn_config_env(env)
    return env
...
if __name__ == "__main__":
    ...
    # Create log dir 
    log_dir = "roundabout_cnn_dqn/"
    os.makedirs(log_dir, exist_ok=True)
    
    # Create a vectorized environment with render_mode="rgb_array"
    env = make_vec_env(roundabout_cnn_train_env, n_envs=n_cpu, vec_env_cls=SubprocVecEnv)
