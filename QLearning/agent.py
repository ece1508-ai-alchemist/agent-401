...
class QLearningAgent:
    def __init__(self, action_space, state_space, alpha=0.1, gamma=0.99, epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.1):
...
def train_agent(num_episodes=2000, save_file=None):
    ...
    for episode in tqdm(range(num_episodes), desc="Training Progress"):
        ...
        if (episode + 1) % 100 == 0:
            tqdm.write(f"Episode {episode + 1}/{num_episodes}, Total Reward: {total_reward}")
...
def test_agent(agent, num_episodes=10, record=False, video_folder="videos"):
    ...
    for episode in tqdm(range(num_episodes), desc="Testing Progress"):
        ...
        tqdm.write(f"Test Episode {episode + 1}/{num_episodes}, Total Reward: {total_reward}")
...
def main():
    ...
    #agent = QLearningAgent(action_space=gym.make('highway-fast-v0').action_space, state_space=gym.make('highway-fast-v0').observation_space)
