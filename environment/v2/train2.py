...
register(id="401-v0", entry_point="environment.v2.highway401:Highway401")
...
if __name__ == "__main__":
    ...
    # Train the model
    if TRAIN:
        model.learn(total_timesteps=int(2000), callback=callback)
    ...
    if TEST:
        ...
        for video in range(10):
            ...
            while not (done or truncated):
                # Predict the actions for both agents
                action, _states = model.predict(obs, deterministic=True)
                # Ensure action is a list of actions for both agents
