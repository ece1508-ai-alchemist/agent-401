...
def roundabout_mlp_config_env(env):
    env.configure(
        {
            "observation": {
...
def roundabout_mlp_train_env():
    env = gym.make("roundabout-v0")
    roundabout_mlp_config_env(env)
    return env
...
if __name__ == "__main__":
    ...
    # Create log dir 
    log_dir = "roundabout_mlp_ppo/"
    os.makedirs(log_dir, exist_ok=True)
    
    # Create a vectorized environment with the original observation space
    env = make_vec_env(roundabout_mlp_train_env, n_envs=n_cpu, vec_env_cls=SubprocVecEnv)
